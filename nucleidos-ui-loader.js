(() => {
  ...
  async function read(paths) {
    ...
    responses.forEach((response, index) => {
      if (!response.ok) throw new Error(`${paths[index]}: HTTP ${response.status}`);
    });
  ...
  async function inflate(encoded) {
    ...
    if (typeof DecompressionStream !== 'function') {
      throw new Error('El navegador no admite descompresión gzip.');
    }
  ...
  Promise.all([read(assets.css), read(assets.js)])
    ...
    .then(([css, js]) => {
      ...
      script.id = 'nucleidos-ui-runtime';
